...
void circle_cent4(float r1, float c1, float r2, float c2, float r3, float c3,
  ...
  double rd, cd;
  double v1 = 2 * (r2 - r1), v2 = 2 * (c2 - c1), v3 = r2 * r2 - r1 * r1
      + c2 * c2 - c1 * c1;
  double v4 = 2 * (r3 - r1), v5 = 2 * (c3 - c1), v6 = r3 * r3 - r1 * r1
      + c3 * c3 - c1 * c1,
...
int de_duplicate(std::vector<Shx> &pts, std::vector<int> &outx) {
  ...
  for (int k = 0; k < nump - 1; k++) {
    if (dpx[k].r == dpx[k + 1].r && dpx[k].c == dpx[k + 1].c) {
      cerr << "duplicate-point ids " << dpx[k].id << "  " << dpx[k + 1].id
           << "   at  (" << pts[dpx[k + 1].id].r << "," << pts[dpx[k + 1].id].c
           << ")" << endl;
...
int Cline_Renka_test(float &Ax, float &Ay, float &Bx, float &By, float &Cx,
  ...
  float v1x = Bx - Ax, v1y = By - Ay, v2x = Cx - Ax, v2y = Cy - Ay, v3x = Bx
      - Dx, v3y = By - Dy, v4x = Cx - Dx, v4y = Cy - Dy;
